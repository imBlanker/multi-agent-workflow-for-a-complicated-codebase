...
Run `mawf plan` for the current project, then read back `.maw/plan.md` and summarize the selected architecture, the agent roster, and any review gates to the user.

Core steps:
1. Run `node ${CLAUDE_PLUGIN_ROOT}/../bin/mawf.js plan --project $PWD $ARGUMENTS` (or `mawf plan --project $PWD $ARGUMENTS` if on PATH).
2. Read `.maw/plan.md`.
...
4. Remind them they can edit any file under `.maw/agents/` and re-run `mawf plan` to regenerate.
