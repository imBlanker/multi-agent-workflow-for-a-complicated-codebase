...
## What you do
1. Read `.maw/plan.md` and `.maw/workflow.json` for the current plan.
...
3. **Before spawning each subagent**, run `maw guard --project $PWD`. Only spawn if it returns ALLOW. On spawn, `maw acquire --id <id> --role <role>`; on completion, `maw release --id <id>`.
...
6. At each review gate in the plan, invoke `/maw:review` (Codex via codex-plugin-cc) if available; otherwise use a second Claude Code agent as reviewer.
